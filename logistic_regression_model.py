...
if __name__ == "__main__":
    ...
    model = LogisticRegression(learning_rate=0.1, max_iter=100, optimizer='gd', fit_intercept=True)

    model.fit(x_train, y_train)
    predictions = model.predict(x_test)

    print("--" * 30, "Logistic Regression (with Gradient Descent) Accuracy score:",
          "Training samples: {0}, AUC on testing set: {1:.3f}".format(700, roc_auc_score(y_test, predictions)),
          "--" * 30,
          sep="\n")
